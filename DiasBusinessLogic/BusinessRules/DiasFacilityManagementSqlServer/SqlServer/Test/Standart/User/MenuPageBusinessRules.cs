...
{
    public class MenuPageBusinessRules : BusinessRuleAbstract, DevelopmentMenuPageInterface.IMenuPageBusinessRules, IBaseMenuPageBusinessRules
    {
        private static AutoMapperProfileMapper<DevelopmentMenuPageProfile.MenuPageProfile> DtoMapper_DiasFacilityManagementSqlServer_Development
            => new  (new (DataContextHelper.GetDataContextTypeViaConfiguration("DiasFacilityManagementSqlServerTest")));

        private readonly IUnitOfWork_EF _unitOfWork_EF;

        public MenuPageBusinessRules() : this(DI_ServiceLocator.Current.Get<IUnitOfWork_EF>())
        {
        }

        private MenuPageBusinessRules(IUnitOfWork_EF unitOfWork_EF)
        {
