...
{
    public class TicketStateFlowRoleBusinessRules : BusinessRuleAbstract, DevelopmentLocationInterface.ITicketStateFlowRoleBusinessRules, IBaseTicketStateFlowRoleBusinessRules
    {
        private static AutoMapperProfileMapper<DevelopmentLocationProfile.TicketStateFlowRoleProfile> DtoMapper_DiasFacilityManagementSqlServer_Development
            => new(new(DataContextHelper.GetDataContextTypeViaConfiguration("DiasFacilityManagementSqlServer")));

        private readonly IUnitOfWork_EF _unitOfWork_EF;

        public TicketStateFlowRoleBusinessRules() : this(DI_ServiceLocator.Current.Get<IUnitOfWork_EF>())
        {
        }

        private TicketStateFlowRoleBusinessRules(IUnitOfWork_EF unitOfWork_EF)
        {
