...
namespace DiasBusinessLogic.BusinessRules.DiasFacilityManagementSql.SqlServer.Development.Standart
{
    public class LocationV2BusinessRules : BusinessRuleAbstract, DevelopmentLocationInterface.ILocationV2BusinessRules, IBaseLocationV2BusinessRules
    {
        ...
        public async Task<Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>> GetAllLocationsAsync()
        {
                    ...
                    case "DiasFacilityManagementSqlServer":
                        {
                            try
                            {
                                ...
                                {
                                    //Şimdilik sadece root ve MH6 alınacak
                                    sonucEntityList = await Task.Run(() =>
                                    DiasFacilityManagementSqlServerContext.LocationV2s.AsQueryable().
                                        Where(x => (x.IsActive.HasValue && x.IsDeleted.HasValue) && (!x.IsDeleted.Value) && (x.IsActive == true)).OrderBy(x=>x.LocationNumber)
                                ...
                                List<CustomLocationDtoForMobileApi> dtoForMobile=new List<CustomLocationDtoForMobileApi>();
                                foreach (var item in convertedDto)
                                {
                                    CustomLocationDtoForMobileApi newItem=new CustomLocationDtoForMobileApi();
                                    newItem.Id=item.Id;
                                    newItem.HierarchyId=item.HierarchyId;
                                    newItem.ParentHierarchy = item.ParentHierarchy;
                                    newItem.LocationNumber=item.LocationNumber;
                                    newItem.LocationName=item.LocationName;
                                    newItem.LocationWithName=item.LocationWithName;
                                    newItem.LocationOriginalName=item.LocationOriginalName;
                                    newItem.LocationDescription=item.LocationDescription;
                                    dtoForMobile.Add(newItem);
                                }
                                return new(Errors.General.GetListSuccess("LocationV2"), dtoForMobile);
                            }
