                                ...
                                using (DiasFacilityManagementSqlServerContext)
                                {
                                               ...
                                               (devExpressRequestObj.RequestOptions.DataSourceLoadOption.Take > 0)))
                                    {
                                        ...
                                        LoadResult filteredOrAndPaginatedResult = await DataSourceLoader.LoadAsync<DevelopmentModel.Ticket>
                                                                                    ...
                                                                                    //.Where(x =>(isAdmin || ((x.TicketAssignedUserId == kullaniciId ||  assignmentGroupId.Contains(x.TickedAssignedAssignmentGroupId ?? -1)) && (ticketStatus.Contains(x.TicketStatusId)))))
                                                                                    //.Where(x => (isAdmin || ((x.TicketAssignedUserId == kullaniciId || assignmentGroupId.Contains(x.TickedAssignedAssignmentGroupId ?? -1)))) || x.AddedByUserId == kullaniciId)
                                ...
                                if (castedDto != null)
                                {
                                    ...
                                    if ((locationHierachyIds != null) && (locationHierachyIds.Length == 1))
                                    {
                                        using (DiasFacilityManagementSqlServerContext)
                                        {
                                            ...
                                            if (!(String.IsNullOrEmpty(locationHierachyId)))
                                            {
                                                DateTime dateFilter = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 6, 0, 0);
                                                if(DateTime.Now.Hour<6)
                                                    dateFilter= new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 0, 0, 0);
                                                // 
                                                //DateTime.Now.AddDays(-1)
                                                sonucEntityList = await Task.Run(() =>
                                                    DiasFacilityManagementSqlServerContext.Tickets.AsQueryable().Where(x => (x.IsDeleted == false) &&
                                                                (x.IsActive == true) && (x.TicketOpenedTime > dateFilter && ((x.TicketStatusId != (int)TicketStatusEnum.CLOSED) || (x.TicketStatusId != (int)TicketStatusEnum.SOLVED))))
                                                        .Include(x => x.TicketRelatedLocations.
                                                                Where(y => (y.IsActive == true && y.IsDeleted == false) && (y.TicketLocation != null && y.TicketLocation.IsActive == true && y.TicketLocation.IsDeleted == false)))
                                                                    .ThenInclude(x => x.TicketLocation)
