...
namespace DiasBusinessLogic.BusinessRules.DiasFacilityManagementSql.SqlServer.Development.Standart
{
    public class BasicTicketBusinessRules : BusinessRuleAbstract, DevelopmentBasicTicketInterface.IBasicTicketBusinessRules, IBaseBasicTicketBusinessRules
    {
        ...
        public async Task<Tuple<Error, DevExpressLoadResultDto<List<DevelopmentDto.BasicTicketDto>>>> GetAllBasicTicketsAsync(DevExpressRequest devExpressRequestObj)
        {
                    ...
                    case "DiasFacilityManagementSqlServer":
                        {
                            try
                            {
                                ...
                                DevExpressLoadResultDto<List<BasicTicketDto>> returnLoadResultDto;
                                using (DiasFacilityManagementSqlServerContext)
                                {
                                               ...
                                               (devExpressRequestObj.RequestOptions.DataSourceLoadOption.Take > 0)))
                                    {
                                        ...
                                        LoadResult filteredOrAndPaginatedResult = await DataSourceLoader.LoadAsync<DevelopmentModel.BasicTicket>
                                                                                    (DiasFacilityManagementSqlServerContext.BasicTickets.AsQueryable()
                                                                                    .Where(x => x.IsDeleted == false && x.IsActive == true)
                                                                                    .Include(x => x.AddedByUser)
                                                                                    .IncludeMultiple(x => x.Attachments, y => y.StateOfBasicTicket, x => x.Tickets, x => x.ReportedUser)
                                                                                    .OrderBy(c => c.AddedTime), devExpressRequestObj.RequestOptions.DataSourceLoadOption);

                                        sonucEntityList = filteredOrAndPaginatedResult.data.Cast<DevelopmentModel.BasicTicket>().ToList();
                                    ...
                                    else
                                    {
                                        ...
                                        .IncludeMultiple(x => x.Attachments,y=>y.StateOfBasicTicket,x=>x.Tickets, x => x.AddedByUser,x=>x.ReportedUser)
                                ...
                                return new(Errors.General.GetListSuccess("BasicTicket"), returnLoadResultDto);
