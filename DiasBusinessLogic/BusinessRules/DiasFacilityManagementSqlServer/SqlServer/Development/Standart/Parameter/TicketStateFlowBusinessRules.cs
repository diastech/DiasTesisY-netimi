                            ...
                            try
                            {
                                ...
                                using (DiasFacilityManagementSqlServerContext)
                                {
                                    sonucEntityList = await Task.Run(() =>
                                        DiasFacilityManagementSqlServerContext.TicketStateFlows.AsQueryable().Where(x => x.IsDeleted == false && x.IsActive == true).Include(x=>x.TicketStateTransitionFlowByTicketStateFlows).ThenInclude(y=>y.TicketStateTransitionByTicketStateTransitionFlow)
