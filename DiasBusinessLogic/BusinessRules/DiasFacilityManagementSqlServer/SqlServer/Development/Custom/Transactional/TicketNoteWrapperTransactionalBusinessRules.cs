...
namespace DiasBusinessLogic.BusinessRules.DiasFacilityManagementSqlServer.SqlServer.Development.Custom
{
    ...
    {        
                    ...
                    case "DiasFacilityManagementSqlServer":
                        {
                            ...
                            using (IDbContextTransaction transaction = DiasFacilityManagementSqlServerContext.Database.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    {
                                        foreach (AttachmentDto attachment in castedDto.Attachments)
                                        {
                                            ...
                                            attachment.FolderName = "Ticket_" + attachment.TicketId.ToString() +
                                                                               "_Added_" + attachment.AddedByUserId.ToString() +
                                                                                "_" + "note_" + DateTime.Now.Ticks.ToString() + fi.Extension;
        ...
        public async Task<Tuple<Error, CustomTicketNoteDto>> DeleteTicketNoteWrapperAsync(BusinessLogicRequest request)
        {
            Type dataContextType = DataContextHelper.GetDataContextTypeViaConfiguration("DiasFacilityManagementSqlServer");
                    ...
                    case "DiasFacilityManagementSqlServer":
                        {
                            ...
                            using (IDbContextTransaction transaction = DiasFacilityManagementSqlServerContext.Database.BeginTransaction())
                            {
                                try
                                {
                                    ...
                                    Tuple<Error, TicketNoteDto> resultAddedTicketNote = await _ticketNoteBusinessRules.DeleteSingleAsync(ticketDto);
                                    ...
                                    if ((resultAddedTicketNote.Item1.BusinessOperationSucceed == true) && (resultAddedTicketNote.Item2 != null))
                                    {
                                        ...
                                        else
                                        {
                                            foreach (var attachment in castedDto.Attachments)
                                            {
                                                ...
                                                Tuple<Error, AttachmentDto> resultDeletedAttachemnt = await _attachmentBusinessRules.DeleteAsync(attachment);

                                                if ((resultDeletedAttachemnt.Item1.BusinessOperationSucceed == true) && (resultDeletedAttachemnt.Item2 != null))
                                                {
                                                    transaction.Commit();
                                                    return new Tuple<Error, CustomTicketNoteDto>(resultAddedTicketNote.Item1, DtoMapper_DiasFacilityManagementSqlServer_Development.Map<TicketNoteDto, CustomTicketNoteDto>(resultAddedTicketNote.Item2));
                                                }
                                                else
                                                {
                                                    ...
                                                    transaction.Rollback();
                                                    return new Tuple<Error, CustomTicketNoteDto>(resultDeletedAttachemnt.Item1, null);
                                                }
                                            }
                                        }
