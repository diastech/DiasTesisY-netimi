...
namespace DiasBusinessLogic.BusinessRules.DiasFacilityManagementSqlServer.SqlServer.Development.Custom
{
    public class LocationWrapperBusinessRules : BusinessRuleAbstract, TransactionalInterface.ILocationWrapperBusinessRules, IBaseLocationWrapperBusinessRules
    {
                    ...
                    case "DiasFacilityManagementSqlServer":
                        {
                            ...
                            Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>> resultGetLocationList = await _locationBusinessRules.GetAllLocationsAsync();
                            try
                            {
                                if ((resultGetLocationList.Item1.BusinessOperationSucceed == true) && (resultGetLocationList.Item2 != null))
                                    return new Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>(resultGetLocationList.Item1, resultGetLocationList.Item2);
                                else
                                    return new Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>(resultGetLocationList.Item1, null);
                            }
                            catch (Exception)
                            {
        ...
        public async Task<Tuple<Error, IEnumerable<CustomDto.CustomLocationDto>>> GetTicketLocationWrapperLastNodeByIdAsync(string hierarchyId)
        {
            Type dataContextType = DataContextHelper.GetDataContextTypeViaConfiguration("DiasFacilityManagementSqlServer");
            if ((dataContextType == null) || (dataContextType.Name == null) || (dataContextType.Name == ""))
            {
            ...
            else
            {
                switch (dataContextType.Name)
                {
                    case "DiasFacilityManagementSqlServer":
                        {
                            DevelopmentContext.DiasFacilityManagementSqlServer DiasFacilityManagementSqlServerContext = new();
                            Tuple<Error, IEnumerable<CustomDto.CustomLocationDto>> resultGetLocationList = await _locationBusinessRules.GetLastNodeTicketLocationByIdAsync(hierarchyId);
