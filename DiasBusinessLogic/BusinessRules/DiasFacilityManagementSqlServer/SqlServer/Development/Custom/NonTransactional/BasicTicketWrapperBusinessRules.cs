...
namespace DiasBusinessLogic.BusinessRules.DiasFacilityManagementSqlServer.SqlServer.Development.Custom
{
    public class BasicTicketWrapperBusinessRules : BusinessRuleAbstract, TransactionalInterface.IBasicTicketWrapperBusinessRules, IBaseBasicTicketWrapperBusinessRules
    {
        ...
        public async Task<Tuple<Error, IEnumerable<CustomDto.CustomBasicTicketDto>>> GetAllBasicTicketsWrapperAsync(DevExpressRequest devExpressRequestObj)
        {
            ...
            {
                return new Tuple<Error, IEnumerable<CustomDto.CustomBasicTicketDto>>(Errors.General.NotFoundDatabaseServer(), null);
            }
            else
            {
                    ...
                    case "DiasFacilityManagementSqlServer":
                        {
                            ...
                            Tuple<Error, List<DevelopmentDto.BasicTicketDto>> resultGetTicketList = await _basicTicketBusinessRules.GetAllBasicTicketsAsync(devExpressRequestObj);
                            ...
                            try
                            {
                                if ((resultGetTicketList.Item1.BusinessOperationSucceed == true) && (resultGetTicketList.Item2 != null))
                                {
                                    ...
                                    return new Tuple<Error, IEnumerable<CustomDto.CustomBasicTicketDto>>(resultGetTicketList.Item1, returnDtoList);
                                }
                                else
                                {
