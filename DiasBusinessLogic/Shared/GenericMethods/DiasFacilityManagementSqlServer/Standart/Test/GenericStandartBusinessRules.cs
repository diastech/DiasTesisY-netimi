...
using BaseDevelopment = DiasDataAccessLayer.DataAccessLayers.EF_Layers.DiasFacilityManagement.SqlServer.Test.BaseModel;
...
namespace DiasBusinessLogic.Shared.GenericMethods.DiasFacilityManagementSqlServer.Standart.Test
{
                                ...
                                using (DiasFacilityManagementSqlServerContext)
                                {
                                    ...
                                    foreach (PropertyInfo pi in typeof(DevelopmentContext.DiasFacilityManagementSqlServer).GetProperties())
                                    {
                                        if (pi.PropertyType == genericListEntityDbSetType)
                                        {
                                            ...
                                            {
                                                if ((uniqueColumns.Count == 1) && (uniqueColumns[0] != null) && (uniqueColumns[0] == "id") &&
                                                    (uniqueValues.Count == 1) && (uniqueValues[0] != null))
                                                {
                                                    //Filtrele(by id)
                                                    existentRecord = await EF_DynamicExpressions.FilterPredicateWithOneParameterSingleOutput("id", uniqueValues[0].ToString(), tEntity, queryableParam, false);
                                                }
                                                else
                                                {
                                    ...
                                    foreach (PropertyInfo pi in typeof(DevelopmentContext.DiasFacilityManagementSqlServer).GetProperties())
                                    {
                                        if (pi.PropertyType == genericListEntityDbSetType)
                                        {
                                            ...
                                            if ((uniqueColumns != null) && (uniqueValues != null))
                                            {
                                                ...
                                                {
                                                    //Filtrele(by id)
                                                    existentRecord = await EF_DynamicExpressions.FilterPredicateWithOneParameterSingleOutput("id", uniqueValues[0].ToString(), tEntity, queryableParam);
                                                }
                                                else
                                                {
                                                    //Filtrele(by unique parameters)
