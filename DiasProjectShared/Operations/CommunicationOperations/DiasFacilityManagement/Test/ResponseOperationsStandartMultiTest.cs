...
{
    /// <summary>
    /// Standart, IEnumerable(list) Dto içeren request response kalıbı
    /// </summary>
    /// <typeparam name="TDto">Dto sınıfı, standart olmak zorunda</typeparam>
    public static class ResponseOperationsStandartMultiTest<TDto> where TDto : BaseDevelopmentStandartDto
    {
                                        ...
                                        bool operationResult = false, string optionalJsonResult = null, string additionalInfo = null)
        {
            switch (serviceResult.Item1) 
            {
                case ErrorCodes.None:
                    {
                        //standart Dtolar
                        return new BusinessLogicResponse((IEnumerable<TDto>)serviceResult.Item2,
