...
namespace DiasWebApi.Repositories.DiasFacilityManagement
{
    public class AssignmentGroupEmployeeDtoRepository : DevelopmentRepositoryInterface.IAssignmentGroupEmployeeDtoRepository
    {
        ...
        public async Task<Tuple<Error, IEnumerable<AssignmentGroupEmployeeDto>>> GetAllAsync()
        {
            if (businessLogicContainerStatus)
            {
                switch (_applicationBusinessLogicEnvironment)
                {
                    case ApplicationBusinessLogicEnvironment.Development:
                        {
                            IAssignmentGroupEmployeeBusinessRules businessRule =
                                (IAssignmentGroupEmployeeBusinessRules)_baseAssignmentGroupEmployeeBusinessRules;

                            return await businessRule.GetAllAssignmentGroupEmployeeAsync();
                        }
            ...
            {
                return new Tuple<Error, IEnumerable<AssignmentGroupEmployeeDto>>(Errors.General.GeneralServerError(), null);
            }
        }

        public async Task<Tuple<Error, IEnumerable<AssignmentGroupEmployeeDto>>> GetUsersByGroupIdAsync(int groupId)
        {
            if (businessLogicContainerStatus)
            {
                switch (_applicationBusinessLogicEnvironment)
                    ...
                    case ApplicationBusinessLogicEnvironment.Development:
                        {
                            ...
                            return await businessRule.GetUsersByGroupId(groupId);
                        }
                    case ApplicationBusinessLogicEnvironment.Test:
                        {
                            return new Tuple<Error, IEnumerable<AssignmentGroupEmployeeDto>>(Errors.General.NotFoundDatabaseServer(), null);
