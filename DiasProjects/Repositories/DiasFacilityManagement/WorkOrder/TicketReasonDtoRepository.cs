...
using DevBLInterface = DiasBusinessLogic.InterfacesAbstracts.Interfaces.BusinessRules.DiasFacilityManagementSqlServer.SqlServer.Development.Standart;
...
namespace DiasWebApi.Repositories.DiasFacilityManagement
{
    public class TicketReasonDtoRepository : DevelopmentRepositoryInterface.ITicketReasonDtoRepository
    {
        ...
        public async Task<Tuple<Error, TicketReasonDto>> GetByIdFromIntAsync(int id)
        {
            if (businessLogicContainerStatus)
            {
                switch (_applicationBusinessLogicEnvironment)
                {
                    ...
                    case ApplicationBusinessLogicEnvironment.Test:
                        {
                            return new Tuple<Error, TicketReasonDto>(Errors.General.NotFoundDatabaseServer(), null);
                        }
                    case ApplicationBusinessLogicEnvironment.Live:
                        {
                            return new Tuple<Error, TicketReasonDto>(Errors.General.NotFoundDatabaseServer(), null);
        ...
        public async Task<Tuple<Error, IEnumerable<TicketReasonDto>>> GetByTicketReasonCategoryIdAsync(int categoryId)
        {
            if (businessLogicContainerStatus)
            {
                switch (_applicationBusinessLogicEnvironment)
                {
                    case ApplicationBusinessLogicEnvironment.Development:
                        {
                            DevBLInterface.ITicketReasonBusinessRules businessRule =
                                (DevBLInterface.ITicketReasonBusinessRules) _baseTicketReasonBusinessRules;

                            return await businessRule.GetTicketReasonsByCategoryIdAsync(categoryId);
                        }
                    case ApplicationBusinessLogicEnvironment.Test:
                        {
                            return new Tuple<Error, IEnumerable<TicketReasonDto>>(Errors.General.NotFoundDatabaseServer(), null);
                        }
                    case ApplicationBusinessLogicEnvironment.Live:
                        {
                            return new Tuple<Error, IEnumerable<TicketReasonDto>>(Errors.General.NotFoundDatabaseServer(), null);
