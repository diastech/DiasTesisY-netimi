...
namespace DiasWebApi.Repositories.DiasFacilityManagement
{
    public class LocationWrapperDtoRepository : ILocationWrapperDtoRepository
    {
        ...
        #region WebClient
        #region Development
        public async Task<Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>> GetAllLocationsWrapperAsync()
        {
            if (businessLogicContainerStatus)
            {
                switch (_applicationBusinessLogicEnvironment)
                {
                    case ApplicationBusinessLogicEnvironment.Development:
                        {
                            ...
                            if (!(String.IsNullOrEmpty(redisCacheKeyHeader)))
                            {
                                redisKey = redisCacheKeyHeader + 
                                    RedisDtoDefinitions.CustomLocationDtoForMobileApi.GetDisplayOrValueFromEnum<RedisDtoDefinitions>();

                                IEnumerable<CustomLocationDtoForMobileApi> locationsFromCache = null;
                                try
                                {
                                    locationsFromCache = await RedisCacheOperations<CustomLocationDtoForMobileApi>.GetFromCacheViaList(_redisCache, redisKey);
                                }
                                catch(Exception)
                                {                                 
                                }
                                ...
                                {
                                    return new Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>(Errors.General.GetListSuccess("LocationV2"), locationsFromCache);
                                }
                            }                           
                            ...
                            Tuple <Error, IEnumerable<CustomLocationDtoForMobileApi>> businessRuleResult = await businessRule.GetAllLocationsWrapperAsync();
                            ...
                            if(businessRuleResult.Item1.BusinessOperationSucceed && (!(String.IsNullOrEmpty(redisCacheKeyHeader))))
                            {
                                ...
                                try
                                {
                                    await RedisCacheOperations<CustomLocationDtoForMobileApi>.SetCacheList(_redisCache, redisKey,
                    ...
                    case ApplicationBusinessLogicEnvironment.Test:
                        {
                            return new Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>(Errors.General.NotFoundDatabaseServer(), null);
                        }
            ...
            {
                return new Tuple<Error, IEnumerable<CustomLocationDtoForMobileApi>>(Errors.General.GeneralServerError(), null);
            }
        }
                    ...
                    case ApplicationBusinessLogicEnvironment.Development:
                        {
                            ...
                            return await businessRule.GetTicketLocationWrapperLastNodeByIdAsync(hierarchyId);
                        }
                    case ApplicationBusinessLogicEnvironment.Test:
                        {
